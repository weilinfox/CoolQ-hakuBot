...
CQ_INIT {
    ...
    on_private_message([](const PrivateMessageEvent &event) {
        if (!event.message.compare(".ping")) {
            try {
                auto msgid = send_private_message(event.user_id, "pong!"); // 直接复读消息
                logging::info_success("私聊ping", "pong!, 消息 Id: " + to_string(msgid));
                //send_message(event.target,
                //             MessageSegment::face(111) + "这是通过 message 模块构造的消息~"); // 使用 message 模块构造消息
            } catch (ApiError &err) {
                //logging::warning("私聊ping", "pong失败!, 错误码: " + to_string(err.code));
            }
        }
        event.block();
    });
    ...
    on_group_message([](const GroupMessageEvent &event) {
        ...
        if (!event.message.compare(".ping")) {
            try {
                auto msgid = send_group_message(event.group_id, "pong!");
